	...
	$.fn.aSendForm = function(params){
		$(this).each(function(){
			var s = $.extend({
				...
				postQuery: '/handle', 		    // Путь к файлу handle.php
				closeData: 1000, 				// Время закрытия окна с ответом
				dataClass: 'amassage_popup', 	// Класс окна с ответом
				associations : null, 			// Объект который ассоциируется с атрибутом name="" в полей формы Например {'name':'Имя', 'phone':'Номер телефона', 'email':Email}
					...
					if (s.onClickForm) {
						...
						if (onClickFormResult) {
							for (var i = 0; i < onClickFormResult.length; i++) {
								...
								//Стало
								input[i+'artsDataD'] = [onClickFormResult[i].title,onClickFormResult[i].data];
							};
						};
					};
                    input['_token'] = $('[name=csrf-token]').attr('content');
                    input['country'] = ['Страна', userCountry ];
